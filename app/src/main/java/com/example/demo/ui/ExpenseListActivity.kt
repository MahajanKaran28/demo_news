...
class ExpenseListActivity : AppCompatActivity(), AlertsAdapter.OnItemClickListener {
    ...
    private fun callApi() {
        ...
        mHashMap["country"] = "us"
        mHashMap["apiKey"] = "9cfb6cba0dad4f2e858996e7037fb0bd"
            ...
            mainViewModel.postStateFlow.collect { it ->
                when (it) {
                    ...
                    is ApiState.Success<*> -> {
                        ...
                        alertsList = result.articles
                        postAdapter.setData(result.articles)
                        postAdapter.notifyDataSetChanged()
                        for (i in 0 until result.articles.size) {
                            //saving to db
                            alertViewModel.insert(result.articles.get(i))
                        }
                    }
                    is ApiState.Empty -> {
